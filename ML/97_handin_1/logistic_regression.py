...
class LogisticRegressionClassifier():
    ...
    def cost_grad(self, X, y, w):
        ...
        # Compute the dot product of w with each row of X
        # [x1^T*w, x2^T*w, ..., xn^T*w]
        # Shape (n,d)X(d,1) = (n,1)
        dot_products = np.dot(X, w)  # Shape (N,), where N is the number of samples

        # Multiply element-wise by -y (vectorized operation)
        # Slide 62 W3
        # 1/n ∑ log(1+e^(-yi*w^t*x_i)
        cost = np.mean(np.log(1+np.exp(-y * dot_products)), axis=0)

        # Compute the gradient for all samples simultaneously
        # Slide 70 W3
        grad = -y[:, np.newaxis] * X * logistic(-y * dot_products)[:, np.newaxis]
    ...
    def fit(self, X, y, w=None, lr=0.1, batch_size=16, epochs=10):
        ...
        """
        # Initialize w (e.g. random N(0,1) distributed entries).
        if w is None: w = np.random.permutation(np.random.rand(X.shape[1]))
        history = []        
        ### YOUR CODE HERE
        # S94 W3
        i = 0
        while i < epochs:
            #Permute (x1,y1),…,(xn,yn) randomly.
            ...
            for j in range(int(X.shape[0] / batch_size)):
                # Let (x1’,y1’),…,(xB’,yB’) be next B data items.
                to_index = min(j + batch_size, X.shape[0]);
                x_batch = X[j:to_index]
                y_batch = y[j:to_index]
            ...
            epoch_cost /= int(X.shape[0] / batch_size)
